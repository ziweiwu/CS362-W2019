...
{
  int i, j;
//  j = state->hand[currentPlayer][choice1];  //store card we will trash [correct line]
  j = state->hand[currentPlayer][choice2];  // bug: wrong variable from parameters because they're named so similarly, so throws away wrong card
...
int playCutpurse(int currentPlayer, struct gameState *state, int handPos)
{
  ...
  //discard played card from hand
//  discardCard(handPos, currentPlayer, state, 0);    // correct line  
  discardCard(currentPlayer, handPos, state, 0);    // bug: int parameters sent in wrong order
  return 0;
}
